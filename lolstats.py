...
print(f"La peticion fue exitosa, Invocador: {usuario_json['gameName']}#{usuario_json['tagLine']}")
...
for matchs in listaMatchs_json:
    ...
    for jugador in listaParticipantes:
        if jugador['puuid'] == usuario_json['puuid']:
            ...
            #df_jugador= build_table(jugador)
    ...
    if responseMatch.status_code == 200:
        ...
        for jugador in listaParticipantes:
            if jugador['puuid'] == usuario_json['puuid']:
                print(f"gano: {jugador['win']}")
                df_jugador= build_table(jugador)
                datos_acumulados.append(jugador)
    else:
...
print("✅ Columnas complejas convertidas a texto para evitar errores de esquema.")
...
print(df_final.head())
df_final.to_excel("sebastian.xlsx",index=False)